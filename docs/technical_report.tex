...
\begin{warnbox}
\textbf{Current research pivot.} This document is retained as historical
background for the older smooth/blended Hybrid LQR-MPC controller. The blended
hybrid method should not be claimed as the final safety-guaranteed publication
method, because direct switching or convex command blending does not guarantee
preservation of MPC obstacle-avoidance constraints. The current proposed method
is the risk-triggered adaptive MPC safety filter with LQR nominal control.
\end{warnbox}
\end{titlepage}
