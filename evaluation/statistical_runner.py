...
from evaluation.scenarios import ObstacleConfig, ScenarioGenerator, get_generator
...
class NoiseConfig:
    ...
    position_noise_std: float = 0.05  # P5-A: Default stochastic noise (meters)
    heading_noise_std: float = 0.01  # P5-A: Default heading noise (radians)
...
class AggregatedResults:
    ...
    metrics: Dict[
        str, Dict[str, float]
    ]  # metric_name -> {mean, std, min, max, p5, p95}
...
def run_single_config(
    ...
    # P4-A: Recalibrated risk influence zone
    ...
    # P4-B: Recalibrated sigmoid blending parameters
    ...
    for k in range(N):
        ...
        if mode == "lqr":
            ...
            x_refs = np.array(
                [traj_gen.get_reference_at_index(k + j)[0] for j in range(lookahead)]
            )
            u_refs = np.array(
                [
                    traj_gen.get_reference_at_index(min(k + j, N - 2))[1][:2]
            ...
            A_d, B_d = linearizer.get_discrete_model_explicit(
                v_ref, traj_gen.get_reference_at_index(k)[0][2]
            )

            sol = mpc.solve_fast(x_measured, x_refs, A_d, B_d, u_refs, obstacle_dicts)
            ...
            if assessment.combined_risk > 0.3:
                ...
                x_refs = np.array(
                    [
                        traj_gen.get_reference_at_index(k + j)[0]
                ...
                sol = mpc.solve_fast(
                    x_measured, x_refs, A_d, B_d, u_refs, obstacle_dicts
                )
                u = sol.optimal_control
                solve_times.append(sol.solve_time_ms)
                if sol.status not in ("optimal", "optimal_inaccurate"):
            ...
            # P1-A: Adaptive NMPC with LMS parameter estimation
            try:
                if not hasattr(run_single_config, "_adaptive_mpc"):
                    # Lazy-init the adaptive MPC (CasADi solver build is slow)
                    run_single_config._adaptive_mpc = AdaptiveMPCController(
                ...
                adaptive_mpc = run_single_config._adaptive_mpc

                lookahead = min(adaptive_mpc.N_ext + 1, N - k)
                x_refs = np.array(
                    [
                        traj_gen.get_reference_at_index(k + j)[0]
                        for j in range(lookahead)
                ...
                sol = adaptive_mpc.solve_tracking(x_measured, x_refs, u_refs, obstacles)
                u = sol.optimal_control
                solve_times.append(sol.solve_time_ms)
                if sol.status not in ("optimal", "fallback"):
                    infeasible_count += 1

                # P1-B: LMS online parameter update
                if k > 0:
                    adaptive_mpc.adapt_parameters(
                        x_measured, states[k - 1], controls[k - 1]
                    )

            except Exception as e:
                # Fallback to LQR if adaptive MPC fails
                u = lqr.compute_control(x_measured, x_ref, x_ref_dot)
        ...
        # -- Check collisions --
        d_safe = 0.35 # Match MPC safety margin
...
def run_statistical_validation(
    ...
    """
    if modes is None:
        modes = ["lqr", "mpc", "hard_switch", "hybrid", "adaptive"]
    ...
    if verbose:
        print(
            f"Generating {n_configs} obstacle configurations (type={scenario_type}, seed={base_seed})..."
        )
            ...
            try:
                ...
                all_results[mode].append(
                    RunMetrics(
                        ...
                        linear_jerk_peak=0.0,
                        angular_jerk_peak=0.0,
                        infeasible_count=999,
                        collision_count=999,
    ...
    for mode in modes:
        ...
        valid_runs = [
            r for r in all_results[mode] if r.mean_tracking_error < float("inf")
        ]
        if valid_runs:
    ...
    for mode, agg in aggregated.items():
        json_data["results"][mode] = {"n_runs": agg.n_runs, "metrics": agg.metrics}

    # P5-A: Validate that results have meaningful stochastic variance
    # before saving, to prevent silently saving deterministic results
    try:
        from evaluation.stats import validate_results_stochastic

        # Check variance across all valid tracking errors
        variance_check = {}
        for mode in modes:
            valid_runs = [
                r for r in all_results[mode] if r.mean_tracking_error < float("inf")
            ...
            if valid_runs:
                variance_check[f"{mode}_tracking"] = np.array(
                    [r.mean_tracking_error for r in valid_runs]
                )
        validate_results_stochastic(variance_check)
    except ValueError as e:
        print(f"\nWARNING: {e}")
        print("Results will still be saved but are flagged as potentially invalid.")
    except ImportError:
        pass  # stats module not available, skip validation
...
def main():
    ...
    parser.add_argument(
        "--configs", type=int, default=50, help="Number of randomized obstacle configs"
    )
    parser.add_argument(
        "--modes",
        nargs="+",
        default=["lqr", "mpc", "hard_switch", "hybrid", "adaptive"],
        choices=["lqr", "mpc", "hard_switch", "hybrid", "adaptive"],
        help="Controller modes to compare",
    )
    parser.add_argument(
        "--duration", type=float, default=20.0, help="Simulation duration (seconds)"
    )
    parser.add_argument(
        "--noise",
        type=float,
        default=0.05,
        help="Position noise std (meters). P5-A default: 0.05",
    )
    parser.add_argument(
        "--heading-noise",
        type=float,
        default=0.01,
        help="Heading noise std (radians). P5-A default: 0.01",
    )
    parser.add_argument(
        "--delay", type=int, default=0, help="Control pipeline delay (timesteps)"
    )
    parser.add_argument(
        "--tau", type=float, default=0.0, help="Actuator time constant (seconds)"
    )
    parser.add_argument(
        "--actuator-noise", type=float, default=0.0, help="Actuator execution noise std"
    )
    parser.add_argument(
        "--scenario",
        type=str,
